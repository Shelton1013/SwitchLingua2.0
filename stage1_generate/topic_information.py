...
class TopicRouter:
    ...
    def fetch(
        ...
        # 1. 检查缓存 — 缓存保存所有候选 snippet，每次随机采样
        cache_key = self._cache_key(topic_id, topic_label)
        cached = self._get_cached(cache_key)
        if cached:
            logger.debug(f"TopicRouter: 命中缓存 ({topic_id}), "
                         f"从 {len(cached)} 条中随机采样 {max_snippets} 条")
            if len(cached) <= max_snippets:
                return list(cached)
            return random.sample(cached, max_snippets)
        ...
        # 3. 按 provider 链尝试，尽量收集更多候选（不再提前终止）
        all_snippets = []
        ...
        # 4. 缓存所有候选 snippet，然后随机采样返回
        if unique_snippets:
            self._set_cache(cache_key, unique_snippets)

        if len(unique_snippets) <= max_snippets:
            return unique_snippets
        return random.sample(unique_snippets, max_snippets)
