...
class TextAnalysis:
    ...
    l1_tokens: list[TokenInfo]      # L1 语言 token
    l2_tokens: list[TokenInfo]      # L2 语言 token
    ...
    matrix_language: str            # 基质语言 (l1_code/l2_code)
    total_content_tokens: int       # 有效内容 token 数（排除标点数字）
    l1_code: str = "zh"             # L1 语言代码
    l2_code: str = "en"             # L2 语言代码
...
class TextAnalyzer:
    ...
    def _tokenize(self, text: str) -> list[TokenInfo]:
        """根据 detection_mode 调度到对应的分词器"""
        if self._detection_mode == "lexicon":
            return self._tokenize_lexicon(text)
        return self._tokenize_unicode(text)

    def _tokenize_unicode(self, text: str) -> list[TokenInfo]:
        """
        Unicode 模式分词（适用于中英、粤英等不同字符集的语言对）：
        - L1（如中文）：每个汉字作为一个 token
        - L2（如英文）：连续字母作为一个 token（即英文单词）
        - 数字：连续数字作为一个 token
        - 标点：每个标点作为一个 token
        - 空白：跳过
        """
    ...
    def _classify_word(self, word: str) -> str:
        """Lexicon 模式：通过词表查找判断单词的语言"""
        if self._l1_lexicon and word.lower() in self._l1_lexicon:
            return self._l1_code
        return self._l2_code

    def _tokenize_lexicon(self, text: str) -> list[TokenInfo]:
        """
        Lexicon 模式分词（适用于德英、法英等同为拉丁字母的语言对）：
        - 按空白/标点切分为词
        - 每个词通过 L1 词表查找判断语言归属
        - 不在词表中的词默认归为 L2
        """
        ...
        while i < len(text):
            ...
            # 字母序列（单词）— 含连字符（如 "well-known"）和撇号（如 "don't"）
            j = i
            while j < len(text) and (text[j].isalpha() or text[j] in "-''" and j + 1 < len(text) and text[j + 1].isalpha()):
                j += 1
            word = text[i:j]
            lang = self._classify_word(word)
    ...
    def _build_segments(self, tokens: list[TokenInfo]) -> list[dict]:
        ...
        只考虑 L1 和 L2，标点和数字归入相邻的语言段。
        """
        if not tokens:
            return []

        content_langs = (self._l1_code, self._l2_code)
        segments = []
        current_lang = None
        current_tokens = []
        start_pos = 0
        ...
        for token in tokens:
            # 标点和数字不影响语言判定，归入当前段
            effective_lang = token.lang if token.lang in content_langs else None
    ...
    def _detect_switch_points(
        ...
        for i in range(1, len(segments)):
            ...
            # 只关注 L1 ↔ L2 的切换
            if prev_seg["lang"] not in content_langs:
                continue
            if curr_seg["lang"] not in content_langs:
                continue
            ...
            # 提取上下文
            prev_tokens = [t.text for t in prev_seg["tokens"] if t.lang in content_langs]
            curr_tokens = [t.text for t in curr_seg["tokens"] if t.lang in content_langs]
    ...
    def analyze(self, text: str) -> TextAnalysis:
        ...
        l1_tokens = [t for t in tokens if t.lang == self._l1_code]
        l2_tokens = [t for t in tokens if t.lang == self._l2_code]
        segments = self._build_segments(tokens)
        sentences = self._split_sentences(text)
        switch_points = self._detect_switch_points(segments, sentences)
        cmi = self._compute_cmi(len(l1_tokens), len(l2_tokens))
        total_content = len(l1_tokens) + len(l2_tokens)
        matrix_lang = self._l1_code if len(l1_tokens) >= len(l2_tokens) else self._l2_code
        ...
        return TextAnalysis(
            ...
            l1_code=self._l1_code,
            l2_code=self._l2_code,
        )
...
class FluencyChecker:
    ...
    def check(self, analysis: TextAnalysis) -> CheckResult:
        ...
        if not analysis.l1_tokens or not analysis.l2_tokens:
            return CheckResult(
        ...
        l1_score = 10.0
        # 检查 L1 片段是否过于碎片化
        l1_segments = [s for s in analysis.segments if s["lang"] == l1]
        single_token_segments = sum(
            1 for s in l1_segments
            if sum(1 for t in s["tokens"] if t.lang == l1) == 1
        )
        if l1_segments and single_token_segments / len(l1_segments) > 0.5:
            l1_score -= 2.0
            violations.append(f"过多单 token L1 碎片 ({single_token_segments}/{len(l1_segments)})")
        details["l1_quality"] = max(0, l1_score)

        # ===== L2 片段质量 (权重 0.4) =====
        l2_score = 10.0
        # 检查 L2 单词是否在常见词表中（宽松检查：容忍未知词，仅当 L2 为英文时）
        unknown_words = []
        for token in analysis.l2_tokens:
            if l2 == "en" and token.text.lower() not in self._COMMON_EN_WORDS and len(token.text) > 1:
                unknown_words.append(token.text)
        # 未知词比例过高才扣分（>50% 的词都不认识 → 可能是乱码）
        # 仅当 L2 为英文时启用此检查（其他语言无内置词表）
        if analysis.l2_tokens and l2 == "en":
            unknown_ratio = len(unknown_words) / len(analysis.l2_tokens)
...
class NaturalnessChecker:
    ...
    def check(self, analysis: TextAnalysis) -> CheckResult:
        ...
        violations = []
        sub_scores = {}
        content_langs = (analysis.l1_code, analysis.l2_code)
        ...
        if analysis.total_content_tokens > 0:
            ...
            content_segments = [
                s for s in analysis.segments if s["lang"] in content_langs
            ]
            if content_segments:
        ...
        l2_segments = [s for s in analysis.segments if s["lang"] == analysis.l2_code]
        if l2_segments:
            # 检查是否有过多单词碎片（非完整短语）
            # 单个 L2 词的段不一定有问题（术语嵌入是正常的）
            # 但如果大量段都是单个词，且不是已知术语/固定表达 → 可能不自然
            very_short = sum(
                1 for s in l2_segments
                if sum(1 for t in s["tokens"] if t.lang == analysis.l2_code) == 1
            )
            if len(l2_segments) > 2 and very_short / len(l2_segments) > 0.8:
                l2_integrity_score -= 2.0
                violations.append("L2 片段过于碎片化，缺少完整短语")
        ...
        if analysis.total_content_tokens > 0:
            dominant_ratio = max(
                len(analysis.l1_tokens), len(analysis.l2_tokens)
        ...
        # 检查词素内部切换
        # Unicode 模式：中文字符紧邻英文字符且无空格/标点分隔
        # Lexicon 模式：跳过此检查（同为拉丁字母，无法从字符级判断）
        raw = analysis.raw_text
        if analysis.l1_code in ("zh", "yue", "ja", "ko"):
            # CJK 语言：检测汉字紧邻拉丁字母
...
class SwitchMotivationChecker:
    ...
    def __init__(self, lang_config=None):
        ...
        # 从 lang_config 加载语言特定的感叹词
        if lang_config:
            self._l1_code = lang_config.l1_code
            self._l2_code = lang_config.l2_code
            self._interjections_l1 = set(
                w.lower() for w in lang_config.interjections_l1
            ) if lang_config.interjections_l1 else self.INTERJECTIONS_ZH
            self._interjections_l2 = self.INTERJECTIONS_EN
        else:
            self._l1_code = "zh"
            self._l2_code = "en"
            self._interjections_l1 = self.INTERJECTIONS_ZH
            self._interjections_l2 = self.INTERJECTIONS_EN
    ...
    def _is_interjection(self, tokens: list[TokenInfo], switch_pos: int) -> bool:
        ...
        for token in tokens:
            if abs(token.position - switch_pos) <= 1:
                if token.lang == self._l2_code and token.text.lower() in self._interjections_l2:
                    return True
                if token.lang == self._l1_code and token.text.lower() in self._interjections_l1:
                    return True
        return False
...
class ProfileConsistencyChecker:
    ...
    _SWITCH_TYPE_EXPECTATIONS = {
        ...
        "ARC_07": {"max_cmi": 0.15, "desc": "Backflag: L2主导，仅L1话语标记，CMI应很低"},
    }
    ...
    _L2_SPAN_EXPECTATIONS = {
        ...
        "ARC_07": {"typical_max": 2, "desc": "L1仅话语标记/感叹词，1-2词"},
    }
    ...
    def check(
        ...
        if archetype_id and archetype_id in self._L2_SPAN_EXPECTATIONS:
            ...
            if l2_segments:
                en_spans = [
                    sum(1 for t in s["tokens"] if t.lang == analysis.l2_code)
                    for s in l2_segments
                ]
...
class DiversityChecker:
    ...
    def _extract_ngrams(self, tokens: list[TokenInfo]) -> set[str]:
        ...
        content = [
            ...
            if t.lang not in ("punct", "num", "other")
        ]
...
class RuleBasedEvaluatorPipeline:
    ...
    def __init__(self, config_path: Optional[str] = None, lang_config=None):
        ...
        self.motivation_checker = SwitchMotivationChecker(lang_config=lang_config)
...
if __name__ == "__main__":
    ...
    print(f"L1 tokens: {len(result.analysis.l1_tokens)}")
    print(f"L2 tokens: {len(result.analysis.l2_tokens)}")
